...
Blockly.Block = function() {
    ...
    goog.asserts.assert(arguments.length == 0, 'Please use Blockly.Block.obtain.');
};
 ...
 * @param {string}
 *            prototypeName The typename of the block.
 */
Blockly.Block.prototype.fill = function(workspace, prototypeName) {
    ...
    if (prototypeName) {
        ...
        goog.asserts.assertObject(prototype, 'Error: "%s" is an unknown language block.', prototypeName);
 ...
 */
Blockly.Block.prototype.mutatorPlus = null;
 ...
 * @param {boolean}
 *            opt_dontRemoveFromWorkspace If true, don't remove this block from
 *            the workspace's list of top blocks.
 */
 ...
 * @param {boolean}
 *            bump Move the unplugged block sideways a short distance.
 */
    ...
    Blockly.doCommand(function() {
        ...
        if (Blockly.selected) {
            ...
            for (var i = 0; !found && i < topBlocks.length; i++) {
                ...
                while (block) {
                    if (block == rootBlock) {
                        // DavidKad: I added the wecode_start_project block as the sole root block
                        // DavidKad: This is a ugly change made by open roberta
                        if (block.type == 'wecode_start_project') {
 ...
 * @param {number}
 *            dy Vertical offset from current location.
 * @private
 */
...
Blockly.Block.prototype.onMouseMove_ = function(e) {
    ...
    Blockly.doCommand(function() {
        if (e.type == 'mousemove' && e.clientX <= 1 && e.clientY == 0 && e.button == 0) {
             ...
             * mousemove events on certain touch actions. Ignore events with
             * these signatures. This may result in a one-pixel blind spot in
             * other browsers, but this shouldn't be noticeable.
             */
            e.stopPropagation();
            return;
        }
 ...
 * 
 * @param {number}
 *            colourHue HSV hue value.
 * @param {number}
 *            colourSaturation HSV saturation value.
 * @param {number}
 *            colourValue HSV value value.
 */
 ...
 * 
 * @param {string}
 *            newValue Value to be the new field.
 * @param {string}
 *            name The name of the field.
 */
...
Blockly.Block.prototype.setPreviousStatement = function(newBoolean, opt_check) {
    if (this.previousConnection) {
        goog.asserts.assert(!this.previousConnection.targetConnection, 'Must disconnect previous statement before removing connection.');
...
Blockly.Block.prototype.setNextStatement = function(newBoolean, opt_check) {
    if (this.nextConnection) {
        goog.asserts.assert(!this.nextConnection.targetConnection, 'Must disconnect next statement before removing connection.');
...
Blockly.Block.prototype.setOutput = function(newBoolean, opt_check) {
    if (this.outputConnection) {
        goog.asserts.assert(!this.outputConnection.targetConnection, 'Must disconnect output value before removing connection.');
 ...
 * 
 * @param {boolean}
 *            newBoolean True if inputs are horizontal.
 */
 ...
 * 
 * @param {?number}
 *            opt_maxLength Truncate the string to this length.
...
Blockly.Block.prototype.interpolateMsg = function(msg, var_args) {
     ...
     *            <string|Blockly.Field>} field This is either a Field or a
     *            tuple of a name and a Field.
     */
    ...
    goog.asserts.assert(dummyAlign === Blockly.ALIGN_LEFT || dummyAlign === Blockly.ALIGN_CENTRE || dummyAlign === Blockly.ALIGN_RIGHT,
    ...
    for (var i = 0; i < tokens.length; i += 2) {
        ...
        if (symbol && symbol.charAt(0) == '%') {
            ...
            goog.asserts.assertArray(tuple, 'Argument "%s" is not a tuple.', symbol);
            if (tuple[1] instanceof Blockly.Field) {
                fields.push([ tuple[0], tuple[1] ]);
            } else {
                input = this.appendValueInput(tuple[0]).setCheck(tuple[1]).setAlign(tuple[2]);
            }
    ...
    if (fields.length) {
        var input = this.appendDummyInput().setAlign(dummyAlign);
        fields.forEach(addFieldToInput, input);
    }
 ...
 * 
 * @param {number}
 *            inputIndex Index of the input to move.
 * @param {number}
 *            refIndex Index of input that should be after the moved input.
 */
 ...
 * @param {boolean}
 *            opt_quiet True to prevent error if input is not present.
 * @throws {goog.asserts.AssertionError}
 ...
 * 
 * @param {Blockly.Mutator}
 *            mutator A mutator dialog instance or null to remove.
 */
