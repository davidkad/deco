...
Blockly.Toolbox.createNodeHtml = function(catName, isSub) {
    ...
    // Davidkad: there should be a way to display an image in a category tab
        ...
        for (var i = 0, childIn; childIn = treeIn.childNodes[i]; i++) {
            ...
            if (name == 'CATEGORY') {
                ...
                if (opt_color) {
                    ...
                    // Davidkad: I added the Wecode categories below 
                    // and removed the oned from open roberta
                    switch (catname) {
                    // if there is no color until now, pick it
                    // TODO get the color variable name from the category name (language
                    // data)
                    case 'Events':
                        Blockly.Toolbox.catcolor = goog.color.rgbArrayToHex(Blockly.CAT_EVENTS_RGB);
                        break;
                    case 'Sprites':
                        Blockly.Toolbox.catcolor = goog.color.rgbArrayToHex(Blockly.CAT_SPRITES_RGB);
                        break;
                    case 'Anim':
                        Blockly.Toolbox.catcolor = goog.color.rgbArrayToHex(Blockly.CAT_ANIM_RGB);
                        break;
                    case 'Logic':
                        Blockly.Toolbox.catcolor = goog.color.rgbArrayToHex(Blockly.CAT_LOGIC_RGB);
                        break;
                    case 'Loops':
                        Blockly.Toolbox.catcolor = goog.color.rgbArrayToHex(Blockly.CAT_LOOPS_RGB);
                        break;
                    case 'Math':
